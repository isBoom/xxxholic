...
type AdminUserListService struct {
	...
	PermissionsType string `json:"permissionsType" form:"permissionsType"`
...
func (s *AdminUserListService) AdminUserList() serializer.Response{
	...
	fmt.Println(s)
	if s.PermissionsType == "admin"{
		if err := model.DB.Where("id in (?)", list).Where(strings.Join(where, "or")).Find(&users).Count(&count).Error; err != nil {
			return serializer.Err(serializer.CodeParamErr,"查询失败",err)
		}
		if err := model.DB.Where("id in (?)", list).Where(strings.Join(where, "or")).Offset(s.Offset).Limit(s.Limit).Find(&users).Error; err != nil {
			return serializer.Err(serializer.CodeParamErr,"查询失败",err)
		}
	}else{
