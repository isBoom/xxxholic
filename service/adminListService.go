...
func (s *AdminListService) AdminList() serializer.Response{
	var list []string
	...
	if err := model.DB.Where("id in (?)", list).Where(strings.Join(where, "or")).Limit(s.Limit).Offset(s.Offset).Find(&users).Error; err != nil {
		return serializer.Err(serializer.CodeParamErr,"查询失败",err)
	}
